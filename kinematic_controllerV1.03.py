...
def setting_up_GPIO():
    ...
    GPIO.add_event_detect(m1_enA, GPIO.RISING, encoder_handler("l"))
    GPIO.add_event_detect(m2_enA, GPIO.RISING, encoder_handler("r"))
...
v = 0.03  # car's speed ##################
ticks_right, ticks_left = 300, 300

#  global variables:
# position variables
# encoder variables:
counter_left, counter_right, counter = 0, 0, 0
prev_counter_right, prev_counter_left = 0, 0
# calculating pose variables:
x0, y0, theta0 = 0,0,0
t = []
counting = []
...
pwr1L, pwr2R = 0, 0
# PID constants
Kp_1, Kp_2, kp = 0.2, 0.2, 0.2
Kd_1, Kd_2, kd = 0.003, 0.003, 0.003
Ki_1, Ki_2, ki = 0.0, 0.0, 0.0
...
setting_up_GPIO()
x_goal = int(input("Enter x goal: "))
y_goal = int(input("Enter y goal: "))

meter_per_ticks_right = (2 * pi * r)/ticks_right
meter_per_ticks_left = (2 * pi * r)/ticks_left
...
# for loop
while abs(x_goal > 0.05) and abs(y_goal > 0.05):
    theta0 = atan2(y0, x0) * 180/pi # converting it to radians


    current_time = time.time()
    time.sleep(0.002) ### try to change it to 0.0002
    delta_time = current_time - prev_time
    delta_right_wheel = counter_right - prev_counter_right
    delta_left_wheel = counter_left - prev_counter_left
    prev_time = current_time
    # don't forget tp add the wl and wr from the list above


    # the PIDs: (here we are trying to control only theta_dot)
    x_error = x_goal - x0
    y_error = y_goal - y0
    theta_goal = atan2(y_error, x_error) * 180/pi
    e = theta_goal - theta0
    e = atan2(sin(e), cos(e))
    dedt = (e - e_prev)/delta_time
    e_integral = e_integral +e
    u = e*kp + dedt*kd + e_integral*ki
    e_prev = e

    wl = (v - (l / 2) * u) / r
    wr = (v + (l / 2) * u) / r
    # the poses:
    Dl = meter_per_ticks_left * delta_left_wheel
    Dr = meter_per_ticks_right * delta_right_wheel
    Dc = (Dl + Dc)/2
    ...
    x0 = x0 + x_dot
    y0 = y0 + y_dot
    theta0 = theta0 + theta_dot


    speed_actual_right = round(delta_right_wheel * 360 / (delta_time * 100 / 3) / 300, 2)
    speed_actual_left = round(delta_left_wheel * 360 / (delta_time * 100 / 3) / 300, 2)
    ...
    speed_output1.append(speed_actual_right)
    speed_output2.append(speed_actual_left)
